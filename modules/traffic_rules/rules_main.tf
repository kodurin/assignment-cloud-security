...
#  NSG / Security rule for win-vm to allow only SSH/RDP traffic from the Azure Bastion 
resource "azurerm_network_security_group" "win-vm-nsg" {
  name                = "win-vm-nsg"
  location            = var.location
  resource_group_name = var.rg-name
...
# Associate the NSG to the win-vm's NIC
resource "azurerm_network_interface_security_group_association" "win-vm-nic-and-nsg-association" {
  network_interface_id      = var.win-vm_nic_id
  network_security_group_id = azurerm_network_security_group.win-vm-nsg.id
}